...
class ArchivosModel extends Query
{
    ...
    {
        $sql = "SELECT * FROM archivos WHERE id_carpeta = :id_carpeta AND estado != 0";
        return $this->selectAll($sql);
 
    }

    public function eliminarCompartido($estado,$fecha, $id)
    {
    ...
    /*public function eliminarCarpeta($estado, $id)
    {
        $sql = "UPDATE carpetas SET estado = ? WHERE id = ? ";
        $array = [$estado, $id];
        return $this->save($sql, $array);
